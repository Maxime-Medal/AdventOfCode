...
lines.map(line => {
  ...
  let game = line.split(':');
  let gameNum = parseInt(game[0].split(" ")[1]);
  ...
  for (let i = 0; i < gameSet.length; i++) {
    ...
    for (let g = 0; g < ga.length; g++) {
      ...
      } else if (color.match(/green/)) {
        green += parseInt(color);
...
console.log("sum", result.reduce((a, b) => a + b));
...
// console.log("Possible Games:", result1);
